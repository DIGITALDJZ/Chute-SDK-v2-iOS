...
static float const kScaleFactor = 0.8;
...
- (CGRect)contentViewFrame {
    return CGRectMake(kSpacingSize, kSpacingSize, self.frame.size.width - kSpacingSize * 2.0, self.frame.size.height - kSpacingSize * 2.0);
}
...
+ (CGRect)popupFrameForView:(UIView *)_view withStartPoint:(CGPoint)_startPoint {
    ...
    float factorWidth = kScaleFactor * _view.frame.size.width;
    float factorHeight = kScaleFactor * _view.frame.size.height;
    float factorX = (_view.frame.size.width - factorWidth) / 2.0;
    float factorY = (_view.frame.size.height - factorHeight) / 2.0;
...
+ (void)showInView:(UIView *)_view fromStartPoint:(CGPoint)_startPoint {
    ...
    GCPopupView *popup = [[self alloc] initWithFrame:popupFrame];
